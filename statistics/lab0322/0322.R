...
var.test(man_math, fem_math) #p-value = 0.8588(>0.05)，表示兩組變異數並無顯著差異
#兩組樣本的變異數相等
...
n2 <- length(B_data$math.score) #所有B成績
...
#p-value = 0.1536(>0.05)，不拒絕H0，表示沒有足夠證據證明AB的數學成績低於60分的比例有顯著差異



#lab1
#調查group A和group B校的數學平均成績是否相同(母體標準差未知，以5%顯著水準進行檢定)
#H0:μA - μB = 0, H1:μA - μB != 0
...
#無法拒絕H0，表示沒有足夠的證據證明兩組的數學平均成績有顯著差異
...
#假設我們的資料集為"樣本資料"，我們想透過檢定得知"有無午餐補助"的閱讀成績高於70分的比例是否有差異
#H0:P男 - P女 = 0, H1:P男 - P女 != 0
...
#p-value = 4.456e-8(<0.05)，拒絕H0，表示有足夠證據證明有無午餐補助的閱讀成績高於70分的比例有顯著差異
