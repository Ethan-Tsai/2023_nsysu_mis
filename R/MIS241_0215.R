...
p1<-data%>% filter(cut == "Premium")%>% select(cut,clarity)%>%head(5)
p1
#mutate()
...
p2<-data%>%mutate(p2sum = price/table)%>%group_by(clarity)%>%summarise(p2avg = mean(p2sum))%>%arrange(desc(p2avg))
p2


## Find the diamond that its price is above 500 USD
## and list the cut and price with the ascending order
p3<-data%>%filter(price>500)%>%select(cut,price)%>%arrange(price)%>%head()
p3
