...
#直方圖(x需為連續型變數)
ggplot(s1) + geom_histogram(aes(x=price), bins = 30) #價格分佈
ggplot(diamonds) + geom_histogram(aes(x=price, fill=color), bins = 30) #加入color的影響
#分組直方圖
ggplot(s1) + geom_histogram(aes(x=price, fill=cut), position="dodge")
#百分比直方圖
...
bbb<-seq(-2,8,0.1)
bbb
q_value<-dnorm(bbb,mean=3,sd=2)
plot(bbb,q_value,     
     type = "l", 
     main = "pdf of the Standard Normal",
     xlab= "Z-score") 
set.seed(123)
rc=rnorm(50,mean=5,sd=2)
hist(rc,main="ccccc")
...
mean.ranget = function(x, alpha=0.05) {
  ...
  r1 = qt(alpha/2, df=n-1) # 左半邊z-score
...
bog=c(25,65, 54, 25, 35, 45, 46, 48, 52, 53
      ,56, 51, 28, 35, 45, 42, 41, 25, 29, 28
      ,29 ,26, 33, 44, 54, 56, 48, 47, 45, 23)
...
#H0<100
set.seed(11)                                         #設計隨機編號
scores <- sample(80:180,30,replace = TRUE)             #成績樣本
scores_mean <- mean(scores)                           #平均成績
scores_n <- 30                                        #樣本數
scores_sd <- sd(scores)  
...
#h0=150
#h1!=150
scores_z_test2<-one.sample.z(scores,                     #放入預檢定資料集
                                null.mu = 150,                 #虛無假設
                                xbar = scores_mean,           #平均值
                                sigma = scores_sd,            #標準差
                                alternative = "two.sided",      #檢定方式(右尾)
                                conf = 0.9 )    
scores_z_test2
#拒絕
...
ans1=c(25, 34,  54,  25,  35,  45,  22,  48,  52,  42
      , 57,  41,  38,  24,  49,  67,  55,  23,  29,  33)
...
#H1!=70
set.seed(20)                                         
ans3 <- sample(0:100,20,replace = TRUE)               
cm_mean <- mean(ans3)                                   
cm_n <- 20                                            

sol3 <- one.sample.z(ans3,                             
                          null.mu = 70,                  
                          xbar = cm_mean,                 
                          sigma = 3,                     
